;;; funcs.el --- haishan Layer packages File for Spacemacs
;;
...
(defun haishan/insert-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ";")))

(defun haishan/delete-semicolon-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ";")
...
(defun haishan/insert-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (insert ",")))

(defun haishan/delete-comma-at-the-end-of-this-line ()
  (interactive)
  (save-excursion
    (end-of-line)
    (if (looking-back ",")
...
(defun haishan/load-my-layout ()
  (interactive)
  (persp-load-state-from-file (concat persp-save-dir "zilong")))

(defun haishan/save-my-layout ()
  (interactive)
...
(defun haishan/indent-region(numSpaces)
  (progn
...
(defun haishan/tab-region (N)
  (interactive "p")
  (if (use-region-p)
      (haishan/indent-region 4)               ; region was selected, call indent-region
    (insert "    ")                   ; else insert four spaces as expected
    ))

(defun haishan/untab-region (N)
  (interactive "p")
  (haishan/indent-region -4))

(defun haishan/hack-tab-key ()
  (interactive)
  (local-set-key (kbd "<tab>") 'haishan/tab-region)
  (local-set-key (kbd "<S-tab>") 'haishan/untab-region)
  )

;; I'm don't like this settings too much.
;; (add-hook 'prog-mode-hook 'haishan/hack-tab-key)
...
(defun haishan/helm-hotspots ()
  "helm interface to my hotspots, which includes my locations,
org-files and bookmarks"
  (interactive)
  (helm :buffer "*helm: utities*"
        :sources `(,(haishan//hotspots-sources))))

(defun haishan//hotspots-sources ()
  ...
  `((name . "Mail and News")
    (candidates . (("Calendar" . (lambda ()  (browse-url "https://www.google.com/calendar/render")))
                   ...
                   ("Run current flie" . (lambda () (haishan/run-current-file)))
...
(defun haishan/browser-refresh--chrome-applescript ()
  (interactive)
  (do-applescript
   (format
    "
...
(defun haishan/open-file-with-projectile-or-counsel-git ()
  (interactive)
  (if (or (haishan/vcs-project-root)
...
(defun haishan/growl-notification (title message &optional sticky)
...
(defun haishan/growl-timer (minutes message)
  ...
  (run-at-time (* (string-to-number minutes) 60)
               ...
               (lambda (minute message)
                 (haishan/growl-notification "Emacs Reminder" message t))
               minutes
               message))

(defun haishan/goto-match-paren (arg)
...
(defun haishan/hidden-dos-eol ()
  "Do not show ^M in files containing mixed UNIX and DOS line endings."
  (interactive)
  (setq buffer-display-table (make-display-table))
  (aset buffer-display-table ?\^M []))

(defun haishan/remove-dos-eol ()
  "Replace DOS eolns CR LF with Unix eolns CR"
  (interactive)
  (goto-char (point-min))
  (while (search-forward "\r" nil t) (replace-match "")))

(defun haishan/insert-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
  (insert (haishan/retrieve-chrome-current-tab-url)))

(defun haishan/retrieve-chrome-current-tab-url()
  "Get the URL of the active tab of the first window"
  (interactive)
...
(defun haishan/single-lines-only ()
...
(defun haishan/named-term (name)
  (interactive "sName: ")
  (ansi-term "/bin/zsh" name))


(defun haishan/ash-term-hooks ()
...
(defun haishan/word-count-for-chinese ()
  "「較精確地」統計中/日/英文字數。
- 文章中的註解不算在字數內。
...
(defun haishan/evil-quick-replace (beg end )
  (interactive "r")
...
(defun haishan/vcs-project-root ()
...
(defun haishan/iterm-shell-command (command &optional prefix)
  "cd to `default-directory' then run COMMAND in iTerm.
With PREFIX, cd to project root."
  (interactive (list (read-shell-command
                      "iTerm Shell Command: ")
                     current-prefix-arg))
  (let* ((dir (if prefix (haishan/vcs-project-root)
                default-directory))
...
(defun haishan/my-mc-mark-next-like-this ()
  (interactive)
  (if (region-active-p)
...
(defun my-open-file-in-external-app (file)
  ...
  (interactive)
  (let ((default-directory (haishan/vcs-project-root))
        (file-path file))
    (if file-path
        (cond
...
(defun haishan/magit-visit-pull-request ()
...
(defun haishan/markdown-to-html ()
  (interactive)
...
(defun haishan/search-in-fireball ()
  (interactive)
  (helm-do-ag (expand-file-name "~/Github/fireball/")))


(defun haishan/show-current-buffer-major-mode ()
  (interactive)
  (describe-variable 'major-mode))

(defun haishan/counsel-imenu ()
  (interactive)
  (counsel-imenu)
  (evil-set-jump))
