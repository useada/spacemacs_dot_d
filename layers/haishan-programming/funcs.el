...
(defun haishan/js2-which-function ()
...
(defun haishan/run-current-file ()
  ...
  (let* (
    ...
    (if (string-equal ξfSuffix "el") ; special case for emacs lisp
      ...
      (if ξprog-name
          (progn
            (message "Running…")
            (async-shell-command ξcmd-str "*haishan/run-current-file output*"))
...
(defun haishan/impatient-mode-hook ()
