...
(defun haishan/octopress-rake (command)
  "run rake commands"
...
(defun haishan/octopress-qrsync (command)
  (let ((command-str (format "/usr/local/bin/qrsync %s" command )))
    (shell-command-to-string command-str)))

(defun haishan/octopress-generate ()
  "generate jekyll site"
  (interactive)
  (haishan/octopress-rake "generate")
  (message "Generate site OK"))

(defun haishan/octopress-deploy ()
  "default deploy task"
  (interactive)
  (haishan/octopress-rake "deploy")
  (haishan/octopress-qrsync "/Users/guanghui/4gamers.cn/guanghui.json")
  (message "Deploy site OK"))

(defun haishan/octopress-gen-deploy ()
  "generate website and deploy"
  (interactive)
  (haishan/octopress-rake "gen_deploy")
  (haishan/octopress-qrsync "/Users/guanghui/4gamers.cn/guanghui.json")
  (message "Generate and Deploy OK"))

(defun haishan/octopress-upimg ()
  (interactive)
  (haishan/octopress-qrsync "/Users/guanghui/4gamers.cn/guanghui.json")
  (message "Up Img to Qiniu"))

(defun haishan/directory-parent (directory)
  (let ((parent (file-name-directory (directory-file-name directory))))
    (if (not (equal directory parent))
        parent)))

(defun haishan/jekyll-serve ()
  (interactive)
  (let* ((default-directory
           (if (string-match "_posts/$" default-directory)
               (haishan/directory-parent (haishan/directory-parent default-directory))
             (haishan/directory-parent default-directory)))
...
;; Screenshot
(defun haishan//insert-org-or-md-img-link (prefix imagename)
  (if (equal (file-name-extension (buffer-file-name)) "org")
      (insert (format "[[%s%s]]" prefix imagename))
    (insert (format "![%s](%s%s)" imagename prefix imagename))))

(defun haishan/capture-screenshot (basename)
  ...
  (if (file-exists-p (file-name-directory fullpath))
      (progn
        ...
        (haishan//insert-org-or-md-img-link "https://haishan.com/img/" relativepath))
    (progn
      (call-process "screencapture" nil nil nil "-s" (concat basename ".png"))
      (haishan//insert-org-or-md-img-link "./" (concat basename ".png"))))
  (insert "\n"))

(defun haishan/org-archive-done-tasks ()
  (interactive)
  (org-map-entries
   (lambda ()
     (org-archive-subtree)
...
(defun haishan/org-archive-cancel-tasks ()
  (interactive)
  (org-map-entries
   (lambda ()
     (org-archive-subtree)
...
(defun haishan/skip-non-stuck-projects ()
...
(defun haishan/org-insert-src-block (src-code-type)
...
(defun haishan/org-save-and-export ()
  (interactive)
